...
class DMTNetwork(nn.Module):
    ...
    def Transformation_Feature(self, query_feats, support_feats, bg_support_feats, prototypes_f, prototypes_b, prototypes_b_q, prototypes_f_q):
        ...
        for idx, feature in enumerate(support_feats):
            ...
            power_R = ((R * R).sum(dim=2, keepdim=True))
            power_R = (power_R+eps).sqrt()
            power_R_q = ((R_q * R_q).sum(dim=2, keepdim=True))
            power_R_q = (power_R_q+eps).sqrt()
            R_q = R_q / (power_R_q + eps)
            R = R / (power_R + eps)
            # R.shape [bsz, 4, ch]
            power_C = ((C * C).sum(dim=2, keepdim=True))
            power_C = (power_C+eps).sqrt()
            
            C = C / (power_C + eps)
            power_C_q = ((C_q * C_q).sum(dim=2, keepdim=True))
            power_C_q = (power_C_q+eps).sqrt()
    ...
    def forward(self, query_img, support_imgs, support_masks):
        return self.predict_mask_1shot(query_img, support_imgs, support_masks)
