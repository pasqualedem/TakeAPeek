...
class Prototype_Adaptive_Module(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "prototype", torch.randn(dim, class_num).requires_grad_(False)
        )
    ...
    def forward(self, x, s_f, s_y, class_idx=None):
        ...
        S = s_f.size(1)
        s_y = F.interpolate(
            s_y, (self.input_resolution[0], self.input_resolution[1]), mode="nearest"
        )
        sup_mask_fg = (
            (s_y == 1).float().reshape(B, S, -1).reshape(B, -1).unsqueeze(-1)
        )  # [B, S * N, 1]
        semantic_prototype, sign_fore_per_batch = self.extract_semantic_prototype(
            s_f, sup_mask_fg
        )
        ...
        if class_idx is not None:
            with torch.no_grad():
                self.train()
                new_semantic_prototype = self.updata_prototype_bank(
                    semantic_prototype, class_idx, sign_fore_per_batch
                )
        else:
            self.eval()
            new_semantic_prototype = self.select_prototype_bank(
                semantic_prototype, self.prototype
            )

        enhanced_feat_q = self.enhanced_feature(
            x.unsqueeze(1), new_semantic_prototype, sign_fore_per_batch
        )
        enhanced_feat_sup = self.enhanced_feature(
            s_f, new_semantic_prototype, sign_fore_per_batch
        )

        registered_feas_q = self.proj_drop(
            self.linears_up(self.act(self.linears_down(enhanced_feat_q.squeeze(1))))
        )
        registered_feas_sup = self.proj_drop(
            self.linears_up(
                self.act(self.linears_down(enhanced_feat_sup.reshape(-1, N, D)))
            )
        )
    ...
    def updata_prototype_bank(self, semantic_prototype, class_idx, sign_fore_per_batch):
        ...
        for i in range(B):
            ...
            if sign_fore_per_batch[i] == 1:
                new_semantic_prototype_per = (
                    self.prototype[:, class_idx_per] * self.momentum
                    + (1 - self.momentum) * semantic_prototype_per
                )
    ...
    def enhanced_feature(self, feature, new_semantic_prototype, sign_fore_per_batch):
        ...
        new_semantic_prototype = nn.functional.normalize(
            new_semantic_prototype, p=2, dim=1
        )
        ...
        similarity_matrix = (
            similarity_matrix * self.dim**0.5
        ) * sign_fore_per_batch.unsqueeze(-1).unsqueeze(-1)

        enhanced_feature = (
            self.act_enhance(similarity_matrix).unsqueeze(-1).repeat(1, 1, 1, D)
            * feature
            + feature
        )
            ...
            for i in range(depth):
                if i % self.gap == 0:
                    self.adapter_blocks.append(
                        Prototype_Adaptive_Module(
                            dim=dim,
                            input_resolution=input_resolution,
                            hidden_ratio=adapter_params["hidden_ratio"],
                            drop=adapter_params["drop_ratio"],
                            class_num=self.class_num,
                            momentum=adapter_params["momentum"],
                        )
                    )

    def forward_query_and_support(
        self, query_img, support_img, support_mask, class_idx
    ):
        """
        ...
        for i, blk in enumerate(self.blocks):
            if self.use_checkpoint:
                ...
                if self.whether_adapter:
                    if i % self.gap == 0:
                        x_adapter_q, x_adapter_sup = self.adapter_blocks[i // self.gap](
                            query_img, support_img, support_mask, class_idx
                        )
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        dpr_adp = [
            x.item() for x in torch.linspace(0, 0.4, sum(depths))
        ]  # stochastic depth decay rule
    ...
    def forward_features_query_and_sup_branch(
        self, query_img, support_img, mask, stack_id, class_idx
    ):
        """
        stack_id [list]
            stack_id = [2, 4, 22, 24]
        ...
        for i, layer in enumerate(self.layers):
            feats_q, feats_sup, query_img, support_img = (
                layer.forward_query_and_support(
                    query_img, support_img, mask, class_idx=class_idx
                )
            )
