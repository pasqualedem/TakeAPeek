...
from .swin_transformer import SwinTransformer, Mlp, DropPath
from ..dcama.transformer import MultiHeadedAttention, PositionalEncoding
import numpy as np
...
class DCAMA_AdaptiveFSS(nn.Module):
    ...
    def __init__(
        ...
        self.adapter_weight = adapter_params.get("adapter_weight", 0.1)
        ...
        if self.backbone == "swin":
            ...
            missing_keys, unexpected_keys = self.feature_extractor.load_state_dict(torch.load(backbone_checkpoint, map_location='cpu')['model'], strict=False)
    ...
    def forward(self, batch, class_idx=None):
        ...
        support_img = support_img.view(B * nshot, C, H, W)
        support_mask = F.interpolate(support_mask, size=(H, W), mode="nearest")
        support_mask_for_adapter = support_mask.view(B * nshot, H, W).unsqueeze(1)
        ...
        if self.backbone == "swin":
            _, feat_maps_q, feat_maps_sup = (
                self.feature_extractor.forward_features_query_and_sup_branch(
                    query_img, support_img, mask, self.stack_ids, class_idx
                )
            )
...
class DCAMA_model(nn.Module):
    def __init__(self, in_channels, stack_ids):
        ...
        for inch in in_channels[1:]:
            self.DCAMA_blocks.append(
                MultiHeadedAttention(h=8, d_model=inch, dropout=0.5)
            )
        ...
        self.conv1 = self.build_conv_block(
            stack_ids[3] - stack_ids[2], [outch1, outch2, outch3], [3, 3, 3], [1, 1, 1]
        )  # 1/32
        self.conv2 = self.build_conv_block(
            stack_ids[2] - stack_ids[1], [outch1, outch2, outch3], [5, 3, 3], [1, 1, 1]
        )  # 1/16
        self.conv3 = self.build_conv_block(
            stack_ids[1] - stack_ids[0], [outch1, outch2, outch3], [5, 5, 3], [1, 1, 1]
        )  # 1/8
        ...
        self.mixer1 = nn.Sequential(
            nn.Conv2d(
                outch3 + 2 * in_channels[1] + 2 * in_channels[0],
                outch3,
                (3, 3),
                padding=(1, 1),
                bias=True,
    ...
    def forward(self, query_feats, support_feats, support_mask, nshot=1):
        ...
        for idx, query_feat in enumerate(query_feats):
            ...
            if nshot == 1:
                ...
                support_feat = torch.stack(
                    [support_feats[k][idx] for k in range(nshot)]
                )
                support_feat = (
                    support_feat.view(-1, ch, ha * wa).permute(0, 2, 1).contiguous()
                )
                mask = torch.stack(
                    [
                        F.interpolate(
            ...
            if idx < self.stack_ids[1]:
                coarse_mask = self.DCAMA_blocks[0](
                    self.pe[0](query), self.pe[0](support_feat), mask
                )
            elif idx < self.stack_ids[2]:
                coarse_mask = self.DCAMA_blocks[1](
                    self.pe[1](query), self.pe[1](support_feat), mask
                )
            else:
                coarse_mask = self.DCAMA_blocks[2](
                    self.pe[2](query), self.pe[2](support_feat), mask
                )
            coarse_masks.append(
                coarse_mask.permute(0, 2, 1).contiguous().view(bsz, 1, ha, wa)
            )
        ...
        coarse_masks1 = (
            torch.stack(
                coarse_masks[
                    self.stack_ids[2]
                    - self.stack_ids[0] : self.stack_ids[3]
                    - self.stack_ids[0]
                ]
            )
            .transpose(0, 1)
        ...
        coarse_masks2 = (
            torch.stack(
                coarse_masks[
                    self.stack_ids[1]
                    - self.stack_ids[0] : self.stack_ids[2]
                    - self.stack_ids[0]
                ]
            )
            .transpose(0, 1)
        ...
        coarse_masks3 = (
            torch.stack(coarse_masks[0 : self.stack_ids[1] - self.stack_ids[0]])
            .transpose(0, 1)
            .contiguous()
            .view(bsz, -1, ha, wa)
        )
