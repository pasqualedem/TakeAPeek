...
from .losses import PairwiseLoss
from timm.models.layers import DropPath, to_2tuple, trunc_normal_
import torch.nn.functional as F
import math, copy

from .vit import vit_model
from .misc import interpb, interpn
from .vit import vit_model as vit_model_original
...
class FPTrans_AdaptiveFSS(nn.Module):
    def __init__(self, opt, adapter_params, **kwargs):
        ...
        self.bg_num = opt.bg_num
        self.shot = opt.shot
        self.benchmark = opt.dataset.lower()
        self.adapter_weight = adapter_params['adapter_weight']
        self.class_num = opt.nways
        ...
        self.encoder = nn.Sequential(OrderedDict([
            ('backbone', vit_model(opt.backbone,
                                       opt.image_size,
                                       pretrained='',
                                       num_classes=0,
                                       depth=opt.vit_depth,
        ...
        # Pretrained model
        self.original_encoder = vit_model_original(opt.backbone,
                                            opt.image_size,
                                            pretrained='',
                                            num_classes=0,
                                            shot=self.shot,
    ...
    def forward(self, x, s_x, s_y, y=None, out_shape=None, class_idx=None):
        ...
        all_cat = (img_cat, self.adapter_block, s_y, self.adapter_weight, class_idx)
    ...
    def load_weights(self, ckpt_path, logger):
        ...
        # Update with original_encoderht
        ...
        assert all("adapter" in k for k in missing_keys), f"Missing not adapter keys in backbone checkpoint: {[m for m in missing_keys if 'adapter' not in m]}"
