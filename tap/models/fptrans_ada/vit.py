...
from . import vit_utils as utils
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', shot=1, original=False, benchmark=None):
        """
        Args:
        ...
        self.patch_embed = embed_layer(
            img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim, stride=None)
        num_patches = self.patch_embed.num_patches

        self.shot = shot
        self.num_prompt = 12 * (1 + 5 * self.shot)
        self.bg_num = 5
        ...
        if self.allow_mod:
            ...
            if benchmark == 'pascal':
                ncls = 15
            if benchmark == 'coco2pascal':
                ncls = 60
            # divider = 1 + opt.bg_num * opt.shot
            divider = 1 + self.bg_num * self.shot
            self.prompt_tokens = nn.Parameter(
                torch.zeros(ncls * divider, self.num_prompt  // divider, embed_dim))  # [bank_size, G, embed_dim]
            nn.init.normal_(self.prompt_tokens.permute(2, 0, 1), std=0.02)
            self.sampler = np.random.RandomState(1234)
            # print(self.prompt_tokens.shape)
    ...
    def forward(self, all_cat):
        if not self.allow_mod:
            return self.forward_original(x)

        x, adapter_block, support_mask, adapter_weight, class_idx = all_cat
        x, (fg_token, bg_token) = x
        bank_size, G, embed_dim = self.prompt_tokens.shape
        S = support_mask.shape[1]
        B = x.shape[0] // (S + 1)
        divider = 1 + self.bg_num * S
        ...
        x = cat_token(x, fg_token, bg_token)#B*(S+1),N+1+n1+n2,C
        ...
        for i, block in enumerate(self.blocks):
            ...
            if i in list_ad:
                for_adapter = x.clone()
                image_token = for_adapter[:,self.num_tokens:N, :]
                image_token_q = image_token[B * S:, :, :]
                image_token_sup = image_token[:B * S, :, :]
                image_token_adapter = adapter_block[index](image_token_q, image_token_sup, support_mask, class_idx)
                image_token = image_token + adapter_weight * image_token_adapter
                token_adapter = torch.cat([for_adapter[:, :self.num_tokens, :], image_token, for_adapter[:, N:, :]], dim=1)
                x = token_adapter
                index += 1
    ...
    def reduce_and_expand(x, num, start=1, end=0):
        B, N, C = x.shape
        x = x.view(B // num, num, N, C) # torch.Size([2, 2, 974, 768])
        if start > 0:
...
@torch.no_grad()
def _load_weights_pth(model: VisionTransformer, checkpoint_path):
...
def vit_model(model_type,
    ...
    return VisionTransformer(img_size=image_size,
                             ...
                             depth=depth or vit_factory[model_type]['depth'],
